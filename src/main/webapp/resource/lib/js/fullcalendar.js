...
DayGrid.mixin({
	...
	fgSegHtml: function(seg, disableResizing) {
		...
		var titleHtml;
		var principal;
		event.task=(event.task!=undefined?event.task:true);
		event.principalColor=(event.principalColor!=undefined?event.principalColor:"#09c");
		event.principal=event.principal?event.principal:"待认领";
		...
		principal=
		'<span class="principal" style="background:'+event.principalColor+'">' +
			(event.principal&&event.principal!="待认领" ? event.principal.charAt(0) : '<span class="icon-user"></span>') +
		'</span>';
		...
		return '<a class="' + classes.join(' ') + '"' +
			...
			'>' +
				...
				(isResizableFromEnd ?
					...
					'<div class="fc-resizer fc-end-resizer" />'  //改动
					) +
			'</a>';
	},
	...
	function render() {
		...
		if (sections) {
			el = $("<div class='fc-toolbar'/>")
				.append('<div class="fc-left"><button class="up-btn up-btn-default calendar_left_btn"><span class="icon-menu"></span></button></div>')
					...
					if (buttonName == 'title') {
						...
						if (buttonClick) {
							...
							button = $( // type="button" so that it doesn't submit a form
								'<button type="button" class="up-btn up-btn-default ' + classes.join(' ') + '">' +
									innerHtml +
								'</button>'
								)
								.click(function() {
